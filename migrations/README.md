...
This directory contains SQLx database migrations for the bucket module. These migrations create a comprehensive database schema with support for:
...
### 001_create_bucket_table.sql
Creates the main `bucket` table with:
...
### 002_create_bucket_events_table.sql
Creates the event store table `bucket_events` with:
...
### 003_create_bucket_audit_table.sql
Creates the audit trail table `bucket_audit` with:
...
### 004_create_bucket_indexes_and_triggers.sql
...
### 005_create_bucket_views.sql
Creates optimized database views:
- `bucket_active` - Only active entities
- `bucket_stats` - Materialized statistics view
- `bucket_with_activity` - Entities with recent activity
- `bucket_audit_summary` - Audit summary per entity
- `bucket_event_timeline` - Chronological event view
- `bucket_audit_timeline` - Chronological audit view
- `bucket_by_metadata` - Entities with extracted metadata
- `bucket_performance_metrics` - Daily performance metrics
...
bucket
├── Main Table: bucket
├── Event Store: bucket_events
├── Audit Trail: bucket_audit
├── Views (Optimized):
│   ├── bucket_active
│   ├── bucket_stats (materialized)
│   ├── bucket_with_activity
│   ├── bucket_audit_summary
│   ├── bucket_event_timeline
│   ├── bucket_audit_timeline
│   ├── bucket_by_metadata
│   └── bucket_performance_metrics (materialized)
└── Functions:
    ├── search_bucket()
    ├── get_bucket_statistics()
    ├── refresh_bucket_materialized_views()
...
CREATE POLICY bucket_read_policy ON bucket
...
FROM pg_matviews
WHERE matviewname LIKE 'bucket_%';

-- Check index usage
SELECT schemaname, tablename, indexname, idx_scan, idx_tup_read
FROM pg_stat_user_indexes
WHERE tablename LIKE 'bucket_%';
```

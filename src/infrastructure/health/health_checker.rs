...
impl DiskSpaceHealthCheck {
    ...
    fn get_disk_usage(path: &str) -> Result<(f64, HashMap<String, serde_json::Value>), String> {
        ...
        details.insert("total_gb".to_string(), serde_json::Value::Number(
            serde_json::Number::from_f64(total_gb).unwrap_or_else(|| serde_json::Number::from(0))
        ));
        details.insert("used_gb".to_string(), serde_json::Value::Number(
            serde_json::Number::from_f64(used_gb).unwrap_or_else(|| serde_json::Number::from(0))
        ));
        details.insert("available_gb".to_string(), serde_json::Value::Number(
            serde_json::Number::from_f64(available_gb).unwrap_or_else(|| serde_json::Number::from(0))
        ));
...
impl CpuHealthCheck {
    ...
    fn get_cpu_usage() -> (f64, HashMap<String, serde_json::Value>) {
        ...
        details.insert("load_average".to_string(), serde_json::Value::Number(
            serde_json::Number::from_f64(1.2).unwrap_or_else(|| serde_json::Number::from(0))
        ));

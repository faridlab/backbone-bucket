//! Bulk Operations for Thumbnail aggregate
//!
...
use crate::domain::repositories::ThumbnailRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for Thumbnail
pub struct ThumbnailBulkOperations<R: ThumbnailRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ThumbnailRepository> ThumbnailBulkOperations<R> {
    ...
    /// Bulk create thumbnail entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Thumbnail>) -> Result<BulkOperationResult<Thumbnail>> {
    ...
    /// Bulk update thumbnail entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Thumbnail>) -> Result<BulkOperationResult<Thumbnail>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete thumbnail entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete thumbnail entities by IDs
    ...
    /// Bulk restore soft-deleted thumbnail entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Thumbnail>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

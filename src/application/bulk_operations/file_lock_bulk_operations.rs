//! Bulk Operations for FileLock aggregate
//!
...
use crate::domain::repositories::FileLockRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for FileLock
pub struct FileLockBulkOperations<R: FileLockRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: FileLockRepository> FileLockBulkOperations<R> {
    ...
    /// Bulk create file_lock entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<FileLock>) -> Result<BulkOperationResult<FileLock>> {
    ...
    /// Bulk update file_lock entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<FileLock>) -> Result<BulkOperationResult<FileLock>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete file_lock entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete file_lock entities by IDs
    ...
    /// Bulk restore soft-deleted file_lock entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<FileLock>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

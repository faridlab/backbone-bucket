//! Bulk Operations for AccessLog aggregate
//!
...
/// Bulk operations handler for AccessLog
pub struct AccessLogBulkOperations<R: AccessLogRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: AccessLogRepository> AccessLogBulkOperations<R> {
    ...
    /// Bulk create access_log entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<AccessLog>) -> Result<BulkOperationResult<AccessLog>> {
    ...
    /// Bulk update access_log entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<AccessLog>) -> Result<BulkOperationResult<AccessLog>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete access_log entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete access_log entities by IDs
    ...
    /// Bulk restore soft-deleted access_log entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<AccessLog>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

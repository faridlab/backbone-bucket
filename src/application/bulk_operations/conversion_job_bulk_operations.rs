//! Bulk Operations for ConversionJob aggregate
//!
...
use crate::domain::repositories::ConversionJobRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for ConversionJob
pub struct ConversionJobBulkOperations<R: ConversionJobRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ConversionJobRepository> ConversionJobBulkOperations<R> {
    ...
    /// Bulk create conversion_job entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<ConversionJob>) -> Result<BulkOperationResult<ConversionJob>> {
    ...
    /// Bulk update conversion_job entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<ConversionJob>) -> Result<BulkOperationResult<ConversionJob>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete conversion_job entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete conversion_job entities by IDs
    ...
    /// Bulk restore soft-deleted conversion_job entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<ConversionJob>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

//! Bulk Operations for ContentHash aggregate
//!
...
use crate::domain::repositories::ContentHashRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for ContentHash
pub struct ContentHashBulkOperations<R: ContentHashRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ContentHashRepository> ContentHashBulkOperations<R> {
    ...
    /// Bulk create content_hash entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<ContentHash>) -> Result<BulkOperationResult<ContentHash>> {
    ...
    /// Bulk update content_hash entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<ContentHash>) -> Result<BulkOperationResult<ContentHash>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete content_hash entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete content_hash entities by IDs
    ...
    /// Bulk restore soft-deleted content_hash entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<ContentHash>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

//! Bulk Operations for UploadSession aggregate
//!
...
use crate::domain::repositories::UploadSessionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for UploadSession
pub struct UploadSessionBulkOperations<R: UploadSessionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UploadSessionRepository> UploadSessionBulkOperations<R> {
    ...
    /// Bulk create upload_session entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UploadSession>) -> Result<BulkOperationResult<UploadSession>> {
    ...
    /// Bulk update upload_session entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UploadSession>) -> Result<BulkOperationResult<UploadSession>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete upload_session entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete upload_session entities by IDs
    ...
    /// Bulk restore soft-deleted upload_session entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UploadSession>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

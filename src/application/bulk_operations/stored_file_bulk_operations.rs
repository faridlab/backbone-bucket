//! Bulk Operations for StoredFile aggregate
//!
...
use crate::domain::repositories::StoredFileRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for StoredFile
pub struct StoredFileBulkOperations<R: StoredFileRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: StoredFileRepository> StoredFileBulkOperations<R> {
    ...
    /// Bulk create stored_file entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<StoredFile>) -> Result<BulkOperationResult<StoredFile>> {
    ...
    /// Bulk update stored_file entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<StoredFile>) -> Result<BulkOperationResult<StoredFile>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete stored_file entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete stored_file entities by IDs
    ...
    /// Bulk restore soft-deleted stored_file entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<StoredFile>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

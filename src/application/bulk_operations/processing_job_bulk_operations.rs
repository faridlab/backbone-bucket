//! Bulk Operations for ProcessingJob aggregate
//!
...
use crate::domain::repositories::ProcessingJobRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for ProcessingJob
pub struct ProcessingJobBulkOperations<R: ProcessingJobRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ProcessingJobRepository> ProcessingJobBulkOperations<R> {
    ...
    /// Bulk create processing_job entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<ProcessingJob>) -> Result<BulkOperationResult<ProcessingJob>> {
    ...
    /// Bulk update processing_job entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<ProcessingJob>) -> Result<BulkOperationResult<ProcessingJob>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete processing_job entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete processing_job entities by IDs
    ...
    /// Bulk restore soft-deleted processing_job entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<ProcessingJob>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

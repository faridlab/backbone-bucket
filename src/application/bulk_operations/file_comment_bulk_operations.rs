//! Bulk Operations for FileComment aggregate
//!
...
use crate::domain::repositories::FileCommentRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for FileComment
pub struct FileCommentBulkOperations<R: FileCommentRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: FileCommentRepository> FileCommentBulkOperations<R> {
    ...
    /// Bulk create file_comment entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<FileComment>) -> Result<BulkOperationResult<FileComment>> {
    ...
    /// Bulk update file_comment entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<FileComment>) -> Result<BulkOperationResult<FileComment>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete file_comment entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete file_comment entities by IDs
    ...
    /// Bulk restore soft-deleted file_comment entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<FileComment>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

//! Bulk Operations for Bucket aggregate
//!
...
/// Bulk operations handler for Bucket
pub struct BucketBulkOperations<R: BucketRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: BucketRepository> BucketBulkOperations<R> {
    ...
    /// Bulk create bucket entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Bucket>) -> Result<BulkOperationResult<Bucket>> {
    ...
    /// Bulk update bucket entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Bucket>) -> Result<BulkOperationResult<Bucket>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete bucket entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete bucket entities by IDs
    ...
    /// Bulk restore soft-deleted bucket entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Bucket>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

//! Bulk Operations for UserQuota aggregate
//!
...
use crate::domain::repositories::UserQuotaRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for UserQuota
pub struct UserQuotaBulkOperations<R: UserQuotaRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserQuotaRepository> UserQuotaBulkOperations<R> {
    ...
    /// Bulk create user_quota entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserQuota>) -> Result<BulkOperationResult<UserQuota>> {
    ...
    /// Bulk update user_quota entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserQuota>) -> Result<BulkOperationResult<UserQuota>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_quota entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_quota entities by IDs
    ...
    /// Bulk restore soft-deleted user_quota entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserQuota>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());

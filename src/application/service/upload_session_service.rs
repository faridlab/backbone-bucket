//! Application service for UploadSession entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `upload_session_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateUploadSessionDto, UpdateUploadSessionDto};
use crate::domain::entity::UploadSession;
use crate::infrastructure::persistence::UploadSessionRepository;

/// Application service for UploadSession entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `upload_session_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct UploadSessionServiceCustom {
///     inner: Arc<UploadSessionService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type UploadSessionService = GenericCrudService<
    UploadSession,
    CreateUploadSessionDto,
    UpdateUploadSessionDto,
    UploadSessionRepository,
>;

// <<< CUSTOM
// END CUSTOM

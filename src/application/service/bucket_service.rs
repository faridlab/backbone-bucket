//! Application service for Bucket entities
//!
//! Generated by backbone-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `bucket_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateBucketDto, UpdateBucketDto};
use crate::domain::entity::Bucket;
use crate::infrastructure::persistence::BucketRepository;

/// Application service for Bucket entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `bucket_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct BucketServiceCustom {
///     inner: Arc<BucketService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type BucketService = GenericCrudService<
    Bucket,
    CreateBucketDto,
    UpdateBucketDto,
    BucketRepository,
>;

// <<< CUSTOM
// END CUSTOM
